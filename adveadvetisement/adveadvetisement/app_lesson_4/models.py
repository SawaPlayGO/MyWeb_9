...
class Marmelad_order(models.Model):
    ...
    images = models.ImageField(verbose_name='Изображение', upload_to="adveadvetisement/")

    class Meta:
        db_table = 'Marmelad_order'

    def __str__(self):
        return f'{self.title} {self.description} {self.auction} {self.price} {self.created_at} {self.update_at} {self.images}'
    ...
    def img_view(self):
        from django.utils import timezone, html
        return html.format_html(
            "<img src='{}' style='width:50px; height:50px' alt='not loaded'>", self.images.url
        )
